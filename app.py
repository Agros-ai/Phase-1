...
# =========================
st.set_page_config(page_title="Agros – Farmer Assistant", layout="wide")
...
df['Pincode'] = df['Pincode'].apply(lambda x: str(int(x)) if pd.notnull(x) else 'Unknown')
...
def llama_local_stream(prompt: str, model: str = "llama2"):
    ...
    try:
        ...
        yield f"Error sending prompt: {e}"
        return
...
with header_l:
    st.image("agros_logo.png", width=82)
with header_r:
    st.markdown("<h1>Agros – Smart Farming Assistant</h1>", unsafe_allow_html=True)
